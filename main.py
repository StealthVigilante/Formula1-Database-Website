...
def createdriver():
    if request.method == 'POST':
        ...
        cursor.execute('''INSERT INTO driver_passwords (Driver_id,Password)
                                  VALUES (%s,'test123')''', (driver_id,))
        connection.commit()
        """
...
def delete_driver():
    ...
    cursor.execute("DELETE FROM driver_passwords WHERE Driver_id = %s", (driver_id_to_delete,))
